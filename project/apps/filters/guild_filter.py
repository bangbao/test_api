...
def create(env):
    ...
    constants = guild_app.constants

    if not name:
        return notices.GUILD_NAME_EMPTY
...
def request(env):
    ...
    if not guild.pk:
        return notices.GUILD_NOT_EXISTS

    free = guild_app.get_member_free(env, guild)
        
    if free < 1:
        return notices.GUILD_IS_FULL
...
def apply(env):
    ...
    if not guild_app.has_guild_perm(user.guild, user, guild_app.PERM_APPLY):
        return notices.NOT_HAS_GUILD_PERM
    ...
    if not member_id in user.guild.apply4:
        return notices.REQUEST_MEMBER_NOT_EXISTS
    ...
    if member.game.info['guild']:
        return notices.USER_IS_A_GUILD_MEMBER
...
def reject(env):
    ...
    if member_id not in user.guild.apply4:
        return notices.NOT_HAS_APPLY_REQUEST
...
def promoted(env):
    ...
    if user_power < power or member_power > user_power:
        return notices.PERM_RANGE_OVERFLOW
...
def quit(env):
    ...
    disband = user.power == guild_app.GUILD_MASTER and \
