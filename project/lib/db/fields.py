...
class ModelDict(dict, ModelItemMixIn):
    ...
    def important(self):
        """
        """
        fields = ('`%s`' % key for key in self.defaults)
        #fields = ",".join(self.defaults.iterkeys())
        #return "`%s`" % fields
        return ','.join(fields)
    ...
    def sql_select(self, name, carrier, full=False):
        """
        """
        fields = '*' if full else self.important
        return "SELECT %s FROM %s_%s WHERE id=%s" % (fields, carrier.NAME, name, carrier.pk), []
...
class ModelList(dict, ModelItemMixIn):
    ...
    def important(self):
        """
        """
        extras = [self.major_name, self.pk_name, 'id']
        fields = ('`%s`' % key for key in itertools.chain(self.defaults.iterkeys(), extras))
    ...
    def sql_select(self, name, carrier, full=False):
        ...
        """
        fields = '*' if full else self.important
        return "SELECT %s FROM %s_%s WHERE %s=%s" % (fields, carrier.NAME, name, 
